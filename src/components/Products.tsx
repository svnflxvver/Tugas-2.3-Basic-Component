function Products() {
  const products = [
    {
      id: "sku2458",
      name: "Fudge - Chocolate Fudge",
      price: 83000,
      image: "http://dummyimage.com/200x200.png/ff4444/ffffff",
      description:
        "Quisque porta volutpat erat. Quisque erat eros, viverra eget, congue eget, semper rutrum, nulla. Nunc purus.",
      isDeleted: false,
      category: "Pre-Order",
      expiryDate: "2022-07-24",
    },
    {
      id: "sku6942",
      name: "Bagelers - Cinn / Brown",
      price: 149000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Quisque id justo sit amet sapien dignissim vestibulum. Vestibulum ante ipsum primis in faucibus orci luctus et ult ",
      isDeleted: "false",
      category: "Ready",
      expiryDate: null,
    },
    {
      id: "sku0163",
      name: "Blueberries - Frozen",
      price: 181000,
      image: "http://dummyimage.com/200x200.png/5fa2dd/ffffff",
      description:
        "In sagittis dui vel nisl. Duis ac nibh. Fusce lacus purus, aliquet at, feugiat non, pretium quis, lectus.",
      isDeleted: false,
      category: "Barang Bekas",
      expiryDate: "2022-10-04",
    },
    {
      id: "sku3286",
      name: "Wine - White, Ej Gallo",
      price: 45000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description: "Fusce consequat. Nulla nisl. Nunc nisl.",
      isDeleted: false,
      category: "Pre-Order",
      expiryDate: null,
    },
    {
      id: "sku6250",
      name: "Table Cloth 54x72 Colour",
      price: 199000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Cras non velit nec nisi vulputate nonummy. Maecenas tincidunt lacus at velit. Vivamus vel nulla eget eros elementu ",
      isDeleted: "false",
      category: "Pre-Order",
      expiryDate: null,
    },
    {
      id: "sku7460",
      name: "Sauce Tomato Pouch",
      price: 487000,
      image: "http://dummyimage.com/200x200.png/5fa2dd/ffffff",
      description:
        "Curabitur in libero ut massa volutpat convallis. Morbi odio odio, elementum eu, interdum eu, tincidunt in, leo. Ma ",
      isDeleted: "false",
      category: "Ready",
      expiryDate: "2022-12-09",
    },
    {
      id: "sku1863",
      name: "Ice Cream - Life Savers",
      price: 451000,
      image: "http://dummyimage.com/200x200.png/ff4444/ffffff",
      description: "Phasellus in felis. Donec semper sapien a libero. Nam dui.",
      isDeleted: false,
      category: "Pre-Order",
      expiryDate: "2022-11-04",
    },
    {
      id: "sku3824",
      name: "Oregano - Dry, Rubbed",
      price: 51000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo. Morbi ut odio.",
      isDeleted: false,
      category: "Barang Bekas",
      expiryDate: null,
    },
    {
      id: "sku5489",
      name: "Tortillas - Flour, 10",
      price: 62000,
      image: "http://dummyimage.com/200x200.png/cc0000/ffffff",
      description:
        "Nullam sit amet turpis elementum ligula vehicula consequat. Morbi a ipsum. Integer a nibh.",
      isDeleted: false,
      category: "Barang Bekas",
      expiryDate: "2022-05-09",
    },
    {
      id: "sku5983",
      name: "Mudslide",
      price: 105000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Etiam vel augue. Vestibulum rutrum rutrum neque. Aenean auctor gravida sem.",
      isDeleted: false,
      category: "Pre-Order",
      expiryDate: "2022-12-23",
    },
    {
      id: "sku2813",
      name: "Bread - Malt",
      price: 292000,
      image: "http://dummyimage.com/200x200.png/cc0000/ffffff",
      description:
        "Curabitur in libero ut massa volutpat convallis. Morbi odio odio, elementum eu, interdum eu, tincidunt in, leo. Ma ",
      isDeleted: "false",
      category: "Barang Bekas",
      expiryDate: null,
    },
    {
      id: "sku9578",
      name: "Wine - Cotes Du Rhone Parallele",
      price: 289000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Maecenas tristique, est et tempus semper, est quam pharetra magna, ac consequat metus sapien ut nunc. Vestibulum a ",
      isDeleted: "false",
      category: "Barang Bekas",
      expiryDate: "2022-06-18",
    },
    {
      id: "sku5656",
      name: "Truffle Shells - Semi - Sweet",
      price: 93000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "In sagittis dui vel nisl. Duis ac nibh. Fusce lacus purus, aliquet at, feugiat non, pretium quis, lectus.",
      isDeleted: false,
      category: "Pre-Order",
      expiryDate: null,
    },
    {
      id: "sku5785",
      name: "Oil - Safflower",
      price: 492000,
      image: "http://dummyimage.com/200x200.png/ff4444/ffffff",
      description:
        "In quis justo. Maecenas rhoncus aliquam lacus. Morbi quis tortor id nulla ultrices aliquet.",
      isDeleted: false,
      category: "Ready",
      expiryDate: "2022-09-29",
    },
    {
      id: "sku3111",
      name: "Longos - Chicken Curried",
      price: 21000,
      image: "http://dummyimage.com/200x200.png/cc0000/ffffff",
      description:
        "Duis bibendum. Morbi non quam nec dui luctus rutrum. Nulla tellus.",
      isDeleted: false,
      category: "Ready",
      expiryDate: null,
    },
    {
      id: "sku9440",
      name: "Napkin White",
      price: 447000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Proin risus. Praesent lectus.",
      isDeleted: false,
      category: "Ready",
      expiryDate: "2022-12-17",
    },
    {
      id: "sku9393",
      name: "Cookies Cereal Nut",
      price: 277000,
      image: "http://dummyimage.com/200x200.png/dddddd/000000",
      description:
        "Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue. Vestibulum ante ipsum primis in faucibus orc ",
      isDeleted: "false",
      category: "Pre-Order",
      expiryDate: null,
    },
    {
      id: "sku3595",
      name: "Filo Dough",
      price: 150000,
      image: "http://dummyimage.com/200x200.png/ff4444/ffffff",
      description:
        "Cras non velit nec nisi vulputate nonummy. Maecenas tincidunt lacus at velit. Vivamus vel nulla eget eros elementu ",
      isDeleted: "false",
      category: "Ready",
      expiryDate: "2022-05-18",
    },
    {
      id: "sku7895",
      name: "Juice - Pineapple, 48 Oz",
      price: 76000,
      image: "http://dummyimage.com/200x200.png/ff4444/ffffff",
      description:
        "Praesent blandit. Nam nulla. Integer pede justo, lacinia eget, tincidunt eget, tempus vel, pede.",
      isDeleted: false,
      category: "Barang Bekas",
      expiryDate: null,
    },
    {
      id: "sku0193",
      name: "Chocolate - Dark",
      price: 367000,
      image: "http://dummyimage.com/200x200.png/5fa2dd/ffffff",
      description:
        "Phasellus sit amet erat. Nulla tempus. Vivamus in felis eu sapien cursus vestibulum.",
      isDeleted: false,
      category: "Barang Bekas",
      expiryDate: "2022-06-05",
    },
  ];

  return (
    <>
      <h1>Products</h1>
      <table className="table">
        <thead>
          <tr>
            <th scope="col">Product Name</th>
            <th scope="col">Description</th>
            <th scope="col">Product Price</th>
            <th scope="col">Category</th>
            <th scope="col">Expiry Date</th>
            <th scope="col">ACTION</th>
          </tr>
        </thead>
        <tbody>
          {products.map((products) => (
            <tr>
              <td key={products.id}>{products.name}</td>
              <td key={products.id}>{products.description}</td>
              <td key={products.id}>{products.price}</td>
              <td key={products.id}>{products.category}</td>
              <td key={products.id}>{products.expiryDate}</td>
              <td></td>
            </tr>
          ))}
        </tbody>
      </table>
    </>
  );
}

export default Products;
